...
def get_args():
    ...
    parser.add_argument('--dataDir', default='D:\\Human_matting\\', help='dataset directory')
    parser.add_argument('--saveDir', default='D:\\Human_matting\\result', help='save result')
    parser.add_argument('--trainData', default='human_matting', help='train dataset name')
    parser.add_argument('--trainList', default='D:\\Human_matting\\portrait_matting\\train.txt', help='train img ID')
    ...
    parser.add_argument('--train_refine', action='store_true', default=True, help='train refine')
    ...
    parser.add_argument('--nEpochs', type=int, default=2100, help='number of epochs to train')
...
def fusion_loss(args, img, mask_gt, seg, alpha_gt, alpha, eps=1e-6):
    ...
    # paper loss    #计算了mask的和rgb的误差
    L_alpha = torch.sqrt(torch.pow(alpha_gt - alpha, 2.) + eps).mean()  #标准均方差损失函数
    
    gt_msk_img = torch.cat((alpha_gt, alpha_gt, alpha_gt), 1) * img    #标签标注的rgb图片
    
    alpha_img = torch.cat((alpha, alpha, alpha), 1) * img      #预测标注的rgb图片
...
def main():
    ...
    train_data = getattr(data, args.trainData) (base_dir = args.dataDir, \
                                                  ...
                                                  patch = args.patch_size)  #getattr这个函数获取属性值
    
    trainloader = DataLoader(train_data, batch_size=args.train_batch, 
                drop_last=True, shuffle=True, num_workers=0, pin_memory=True) # args.nThreads
    ...
    for epoch in range(start_epoch, args.nEpochs+1):
        ...
        if epoch % args.save_epoch == 0:
            ...
            .format(epoch, args.nEpochs, 
                lr, loss_tr, L_alpha_, L_color_, L_cross_)
            print(log)
            t1 = time.time()
